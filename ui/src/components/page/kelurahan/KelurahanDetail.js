...
class KelurahanDetail extends Component {
  ...
  async componentDidMount() {
    ...
    if (param === "tambah") {
      ...
      try {
        const response = await kelompokApi.getKelurahanByKode(
          param,
          keycloak.token
        );
        const kelurahan = response.data;
        const kodeKecamatan = kelurahan.kecamatan
          ? kelurahan.kecamatan.kodeKecamatan
          : "";
        const form = {
  ...
  isValidForm = async () => {
    ...
    if (form.kodeKelurahan.trim() === "") {
      ...
      form.kodeKelurahanError = {
        pointing: "below",
        content: "Kode Kelurahan harus diisi"
      };
    } else if (form.kodeKelurahan.length !== 9) {
      kodeKelurahanError = true;
      form.kodeKelurahanError = {
        pointing: "below",
        content: "Kode Kelurahan harus 9 digit"
      };
    } else if (
      form.kecamatan.kodeKecamatan !== form.kodeKelurahan.substr(0, 6)
    ) {
      kodeKelurahanError = true;
      form.kodeKelurahanError = {
        pointing: "below",
        content:
          "Kode Kelurahan harus diawali dengan " + form.kecamatan.kodeKecamatan
      };
    } else {
      if (this.props.match.params.kodeKelurahan === "tambah") {
        try {
          ...
          form.kodeKelurahanError = {
            pointing: "below",
            content:
              "Kode Kelurahan sudah terpakai oleh Kelurahan " +
    ...
    if (form.kodeKelurahanCapil.trim() === "") {
      ...
      form.kodeKelurahanCapilError = {
        pointing: "below",
        content: "Kode Kelurahan Kemendagri harus diisi"
      };
    } else if (form.kodeKelurahanCapil.length !== 10) {
      kodeKelurahanCapilError = true;
      form.kodeKelurahanCapilError = {
        pointing: "below",
        content: "Kode Kelurahan Kemendagri harus 10 digit"
      };
    } else {
      if (this.props.match.params.kodeKelurahan === "tambah") {
        try {
          const { keycloak } = this.props;
          const response = await kelompokApi.getKelurahanByKodeCapil(
            form.kodeKelurahanCapil,
            keycloak.token
          );
          const kelurahan = response.data;
          kodeKelurahanCapilError = true;
          form.kodeKelurahanCapilError = {
            pointing: "below",
            content:
              "Kode Kelurahan Capil sudah terpakai oleh Kelurahan " +
              kelurahan.namaKecamatan
          };
        } catch (error) {
          handleLogError(error);
          kodeKelurahanCapilError = false;
    ...
    if (form.namaKelurahan.trim() === "") {
      ...
      form.namaKelurahanError = {
        pointing: "below",
        content: "Nama Kelurahan harus diisi"
      };
    }
    if (form.kecamatan.kodeKecamatan.trim() === "") {
      ...
      form.kecamatanError = {
        pointing: "below",
        content: "Kecamatan harus dipilih"
      };
    }
    this.setState({ form });
    return !(
      kodeKelurahanError ||
      kodeKelurahanCapilError ||
      namaKelurahanError ||
      kecamatanError
    );
  };
  handleActionModal = async (response) => {
    if (response) {
      ...
      try {
        await kelompokApi.deleteKelurahan(
          deleteKelurahan.kodeKelurahan,
          keycloak.token
        );
        toast.info(
          <div>
            <p>
              Kelurahan {deleteKelurahan.namaKelurahan} telah dihapus, Mohon
              Tunggu...
            </p>
          </div>,
  ...
  handleSaveKelurahan = async () => {
    ...
    try {
      ...
      toast.error(
        <div>
          <p>Ada Kesalahan, Silahkan Periksa Kode Kelurahan</p>
        </div>,
        { onClose: () => this.setState({ isLoadingForm: false }) }
      );
      handleLogError(error);
  ...
  handleClickBack = () => this.props.history.push("/kelurahan");
      ...
      return (
        <Container className="isi" text>
          {this.props.match.params.kodeKelurahan === "tambah" ? (
            <Header as="h1" textAlign="center"> Tambah Kelurahan </Header>
          ) : (
            <Header as="h1" textAlign="center"> {form.namaKelurahan} </Header>
          )}
          ...
          <Form loading={isLoadingForm}>
            ...
            {this.props.match.params.kodeKelurahan !== "tambah" ? (
              <Button
                negative
                floated="left"
                onClick={() => this.handleDeleteKelurahan(form)}
              >
                Hapus
              </Button>
            ) : (
            ...
            <Button
              ...
              floated="right"
              type="submit"
              onClick={this.handleSaveKelurahan}
            >
              Simpan
            </Button>
          </Form>

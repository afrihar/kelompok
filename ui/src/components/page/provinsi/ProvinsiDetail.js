...
class ProvinsiDetail extends Component {
  ...
  async componentDidMount() {
    ...
    if (param === "tambah") {
      ...
      try {
        const response = await kelompokApi.getProvinsiByKode(
          param,
          keycloak.token
        );
  ...
  isValidForm = async () => {
    ...
    if (form.kodeProvinsiCapil.trim() === "") {
      ...
      try {
        if (this.props.match.params.kodeProvinsi === "tambah") {
          const { keycloak } = this.props;
          const response = await kelompokApi.getProvinsiByKodeCapil(
            form.kodeProvinsiCapil,
            keycloak.token
          );
          const provinsi = response.data;
          kodeProvinsiCapilError = true;
  ...
  handleActionModal = async (response) => {
    if (response) {
      ...
      try {
        await kelompokApi.deleteProvinsi(
          deleteProvinsi.kodeProvinsi,
          keycloak.token
        );
        toast.info(
          <div>
            <p>
              Provinsi {deleteProvinsi.namaProvinsi} telah dihapus, Mohon
              Tunggu...
            </p>
          </div>,
  ...
  handleSaveProvinsi = async () => {
    ...
    try {
      ...
      toast.error(
        <div>
          <p>Ada Kesalahan, Silahkan Periksa Kode Provinsi</p>
        </div>,
        { onClose: () => this.setState({ isLoadingForm: false }) }
      );
      handleLogError(error);

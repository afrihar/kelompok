    ...
    <Container className="isi">
      {kc.keycloak.authenticated && (
        <Header as="h2" icon textAlign="center">
          <Header.Content>
            Selamat Datang {kc.keycloak.tokenParsed["given_name"]}{" "}
            {kc.keycloak.tokenParsed["family_name"]}
          </Header.Content>
        </Header>
      )}
      {kc.keycloak.authenticated && kc.keycloak["avatar"] && (
        <Image
          size="tiny"
          centered
          circular
          src={getAvatarUrl(kc.keycloak["avatar"])}
        />
      )}
      {kc.keycloak.authenticated && (
      ...
      {!kc.keycloak.authenticated && (
        <Header as="h1" className="ui center aligned header">
          Sistem Informasi Kelompok Dasawisma
        </Header>
      )}
    </Container>
  );

...
public class PetugasByDomisiliSpecification implements Specification<Petugas> {
  private final Petugas filteredPetugas;

  public PetugasByDomisiliSpecification(Petugas filteredPetugas) {
  ...
  private Predicate buildFilterByString(Root<Petugas> root, CriteriaBuilder cb) {
    return cb.or(
        ...
        cb.like(cb.lower(root.get("nama")), "%" + filteredPetugas.getNama().toLowerCase() + "%"),
  ...
  private Predicate buildFilterByWilayah(Root<Petugas> root, CriteriaBuilder cb) {
    return cb.like(root.get("rtDomisili").get("kodeRt"), filteredPetugas.getRtTugas().getKodeRt() + "%");
  }

  private Predicate buildFilterByPrincipal(Root<Petugas> root, CriteriaBuilder cb) {
    return cb.like(root.get("rtDomisili").get("kodeRt"), PrincipalUtil.getKodeWilayah() + "%");
  }
  ...
  public Predicate toPredicate(@SuppressWarnings("NullableProblems") Root<Petugas> root,
    ...
    if (PrincipalUtil.isPusdatin()) {
      if (filteredPetugas.getRtTugas() != null && filteredPetugas.getRtTugas().getKodeRt() != null) {

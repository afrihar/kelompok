...
import { getAvatarUrl, isKecamatan, isKelurahan, isKota, isProvinsi, isPusdatin, isRt, isRw } from './Helpers'
...
function Navbar (props) {
  ...
  const getMenuMasterKota = () => {
    return keycloak.authenticated && (isPusdatin(keycloak) || isProvinsi(keycloak)) ?
      <Dropdown.Item as={NavLink} exact to="/kota" onClick={checkAuthenticated}>Kota</Dropdown.Item> : <></>
  }
  const getMenuMasterKecamatan = () => {
    return keycloak.authenticated && (isPusdatin(keycloak) || isProvinsi(keycloak) || isKota(keycloak)) ?
  ...
  const getMenuMasterKelurahan = () => {
    return keycloak.authenticated && (isPusdatin(keycloak) || isProvinsi(keycloak) || isKota(keycloak) || isKecamatan(keycloak)) ?
      <Dropdown.Item as={NavLink} exact to="/kelurahan" onClick={checkAuthenticated}>Kelurahan</Dropdown.Item> : <></>
  }
  const getMenuMasterRw = () => {
    return keycloak.authenticated && (isPusdatin(keycloak) || isProvinsi(keycloak) || isKota(keycloak) || isKecamatan(keycloak) || isKelurahan(keycloak)) ?
  ...
  const getMenuKelompok = () => {
    return keycloak.authenticated && (isPusdatin(keycloak)) ?
      <Dropdown.Item as={NavLink} exact to="kelompok" onClick={checkAuthenticated}>Kelompok</Dropdown.Item> : <></>
  }
  const getMenuBangunan = () => {
    return keycloak.authenticated && (isPusdatin(keycloak)) ?
      <Dropdown.Item as={NavLink} exact to="bangunan" onClick={checkAuthenticated}>Bangunan</Dropdown.Item> : <></>
  }
  const getMenuRumahTangga = () => {
    return keycloak.authenticated && (isPusdatin(keycloak)) ?
      <Dropdown.Item as={NavLink} exact to="rumah-tangga" onClick={checkAuthenticated}>Rumah
        Tangga</Dropdown.Item> : <></>
  }
  const getMenuKeluarga = () => {
    return keycloak.authenticated && (isPusdatin(keycloak)) ?
      <Dropdown.Item as={NavLink} exact to="keluarga" onClick={checkAuthenticated}>Keluarga</Dropdown.Item> : <></>
  }
  const getMenuIndividu = () => {
    return keycloak.authenticated && (isPusdatin(keycloak)) ?
      <Dropdown.Item as={NavLink} exact to="individu" onClick={checkAuthenticated}>Individu</Dropdown.Item> : <></>
  }
  return (
        ...
        <Menu fixed='top'>
          <Container>
            ...
            <Menu.Menu position='right'>
              {keycloak.authenticated &&
              <Menu.Item> <img src={getAvatarUrl(keycloak['avatar'])} alt={'avatar'}/> </Menu.Item>}
              {keycloak.authenticated &&
              <Dropdown inline text={`${getFirstname()}  ${getLastname()}`} pointing className={'link item'}>
                <Dropdown.Menu>

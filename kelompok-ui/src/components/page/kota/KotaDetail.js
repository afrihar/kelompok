    ...
    if (param === 'tambah') {
      ...
      try {
        ...
        const form = {
          ...
          provinsi: { kodeProvinsi: kota.provinsi.kodeProvinsi },
          kodeKotaError: false,
          kodeKotaCapilError: false,
          namaKotaError: false
        }
  ...
  isValidForm = async () => {
    ...
    if (form.kodeKota.trim() === '') {
      ...
      if (this.props.match.params.kodeKota === 'tambah') {
        try {
          const { keycloak } = this.props
          const response = await kelompokApi.getKotaByKode(form.kodeKota, keycloak.token)
          const kota = response.data
          kodeKotaError = true
          form.kodeKotaError = {
            pointing: 'below',
    ...
    if (form.kodeKotaCapil.trim() === '') {
      ...
      form.kodeKotaCapilError = { pointing: 'below', content: 'Kode Kota Kemendagri harus diisi' }
    } else if (form.kodeKotaCapil.length !== 4) {
      kodeKotaCapilError = true
      form.kodeKotaCapilError = { pointing: 'below', content: 'Kode Kota Kemendagri harus 4 digit' }
    } else {
      if (this.props.match.params.kodeKota === 'tambah') {
        try {
          const { keycloak } = this.props
          const response = await kelompokApi.getKotaByKodeCapil(form.kodeKotaCapil, keycloak.token)
          const kota = response.data
          kodeKotaCapilError = true
          form.kodeKotaCapilError = {
            pointing: 'below',
            content: 'Kode Kota Capil sudah terpakai oleh Kota ' + kota.namaKota
          }
        } catch (error) {
          handleLogError(error)
          kodeKotaCapilError = false
    ...
    this.setState({ form })
    return (!(kodeKotaError || kodeKotaCapilError || namaKotaError || provinsiError))
  }
          ...
          <Form loading={isLoadingForm}>
            <Form.Field required>
              ...
              <Form.Dropdown
                ...
                selection
                value={form.provinsi.kodeProvinsi === '' ? '31' : form.provinsi.kodeProvinsi}
              />
            </Form.Field>
            <Form.Field required>
